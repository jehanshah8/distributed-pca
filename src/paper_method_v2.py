...
def TestEvaluationMetric(P_t, P_t_hat, n_components):
    """ Jaccard Distance:
        P_t : (n x t)
        P_t_hat : (n x t)
    """
    ...
    for j in range(0,P_t.shape[0]): # for each reduced dimensions
        sum_max = 0
        for i in range(0,n_components): # for each sample 
            sum_max += (abs(P_t[j,i]) - abs(P_t_hat[j,i]))**2
            
        Jaccard_Distance.append( sum_max**0.5 )

    return sum(Jaccard_Distance)
    
def TestEvaluationMetric3(Cov_Central, Cov_Dist, n_components):
    Uc, Dc, E_Tc = linalg.svd(Cov_Central, full_matrices=True)
    Ud, Dd, E_Td = linalg.svd(Cov_Dist, full_matrices=True)
    print(Dc)
    print(Dd)
    ...
    print(Percent_Exp_Variance_Central, Percent_Exp_Variance_Dist)
...
if __name__ == "__main__":
    ...
    JD = TestEvaluationMetric(projected_2, projected_1, n_components)
    print(JD)
